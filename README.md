...
##***🛠️ Tools Used -*** : - *Primary:* Linux Command Line Interface (CLI) 
                            - *Alternative:* `systemctl` GUI (e.g., GNOME Services)

##***🧾 Hints / Mini Guide*** :  1. *List running processes* using `ps`, `top`. 
                                 2. *Kill processes* using `kill` and `kill -9`. 
                                 3. *Understand process states* (R, S, D, T, Z). 
                                 4. *Start and stop services* using `systemctl`. 
                                 5. *Enable services at boot* using `systemctl enable`. 
                                 6. *Monitor resource usage* (CPU, memory, I/O, network). 
                                 7. *Record findings* in a structured report.
##***🎯 Deliverables*** : - *Process Management Report* (`process_management_report.md`)
